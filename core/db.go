...
var schema = `
	...
	CREATE TABLE IF NOT EXISTS jobs (
		...
		recorded_at STRING DEFAULT CURRENT_TIMESTAMP,
		txID STRING,
		streamID STRING,
		segmentCost INTEGER,
...
func (db *DB) Close() {
	glog.V(common.DEBUG).Info("Closing DB")
	if db.updateKV != nil {
		db.updateKV.Close()
	}
	if db.insertJob != nil {
...
func (db *DB) SetLastSeenBlock(block *big.Int) error {
	glog.V(common.DEBUG).Info("db: Setting LastSeenBlock to ", block)
...
func (db *DB) InsertJob(job *DBJob) error {
	glog.V(common.DEBUG).Info("db: Inserting job ", job.ID)
...
func (db *DB) ActiveJobs(since *big.Int) ([]*DBJob, error) {
	glog.V(common.DEBUG).Info("db: Querying active jobs since ", since)
...
func (db *DB) SetStopReason(id *big.Int, reason string) error {
	glog.V(common.DEBUG).Infof("db: Setting StopReason for %v to %v", id, reason)
